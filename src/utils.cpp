...
void paste_clipboard() {
    ...
    // system("powershell.exe -c \"Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait('^v')\"");
    #endif
}
...
void notify_user(const std::string& title, const std::string& message) {
    ...
    system(cmd.c_str());
    #elif __APPLE__
    std::string cmd = "osascript -e 'display notification \"" + message + "\" with title \"" + title + "\"'";
    system(cmd.c_str());
    #elif _WIN32
